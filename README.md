...
| 18 | `:postfix_increment_unchecked` | [`++i/i++` should be `unchecked{++i}`/`unchecked{i++}` when it is not possible for them to overflow](https://github.com/seeu-inspace/solidityinspector/wiki#ii-should-be-unchecked-iuncheckedi-when-it-is-not-possible-for-them-to-overflow) | Gas |
